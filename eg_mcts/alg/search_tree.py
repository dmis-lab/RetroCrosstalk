...
class SearchTree:
    ...
    def _add_reaction_and_mol_nodes(self, pro, mols, parent, init_value, ancestors=[]):
        assert pro >= 0
        # repeat : ignored
        for mol in mols:
            if mol in ancestors:
        ...
        reaction_node = ReactionNode(parent, pro, init_value)
        succe = True
        for mol in mols:
    ...
    # expand phase
    def expand(self, mol_node, reactant_lists, pros):
        ...
        if pros is None:
            ...
            for i in range(len(pros)):
                # experience guidance network
                init_value = self.value_fn(mol_node.mol)
                self.value_model_call += 1
                node = self._add_reaction_and_mol_nodes(pros[i], reactant_lists[i], mol_node, init_value, ancestors)
