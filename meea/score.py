"""
Reaction Feasibility and Yield Prediction Evaluation

This script evaluates the feasibility and yield of predicted chemical reactions from various
retrosynthesis models. It handles different datasets and model outputs, calculating success rates,
feasibility scores, and yield predictions.

Usage:
    python evaluate_reactions.py --tree [TREE_TYPE] --ss_model [MODEL_NAME] --dataset [DATASET_NAME]

Args:
    --tree: Tree search algorithm (e.g., 'MEEA')
    --ss_model: Single-step model name (e.g., 'default', 'AZF', 'LocalRetro', etc.)
    --dataset: Dataset name (e.g., 'n1', 'n5', 'retro190', etc.)
"""
...
def parse_reaction(smiles, delimiter1=".", delimiter2=">>"):
    """
    Parse reaction SMILES into reactants and product.
    ...
    Returns:
        Tuple of (reactants_list, product) or (None, None) if parsing fails
    """
    if delimiter2 in smiles:
        reactants_str, product = smiles.split(delimiter2)
        if delimiter1 in reactants_str:
            reactants = reactants_str.split(delimiter1)
        else:
            reactants = [reactants_str]
        return reactants, product
    return None, None

def convert_meea_route(routes):
    """
    Convert MEEA routes to reaction list format.
    
    Args:
        routes: List of route SMILES strings
        
    Returns:
        List of (reactants, product) tuples or None if routes is empty or has only one element
    """
    if not routes or len(routes) <= 1:
        return None
    return [parse_reaction(route) for route in routes[1:]]
...
def prepare_yield_model():
    ...
    adjusted_state = {k.replace("model.", "", 1): v for k, v in state['state_dict'].items() 
...
def standardize_pred_keys(pred_file):
    ...
    # Time (already consistent)
    standardized['time'] = pred['time']
...
def evaluate_predictions(pred_list, dataset, args, model_name):
    ...
    stats = {
        ...
        'total_reactions': 0,
        'total_rfm_score': [0] * total_predictions
    }
    ...
    for pred_file in tqdm(pred_list, desc="Processing prediction files"):
        ...
        for i, route in enumerate(pred['routes']):
            ...
            if success == 1 and time < DEFAULT_TIMEOUT:
                reactions_list = convert_meea_route(route)
                if reactions_list:
                    try:
    ...
    json_path = f'{save_dir}/{model_name}_{dataset}.json'
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Evaluate reaction predictions')
    parser.add_argument('--tree', '-t', default='MEEA', help='Tree search algorithm')
    parser.add_argument('--ss_model', '-ss', choices=['default', 'AZF', 'LocalRetro', 'Chemformer', 'ReactionT5'],
                        help='Single-step model name')
    parser.add_argument('--dataset', '-d', choices=['n1', 'n5', 'retro190', 'drugbank', 'literature30', 'patent200'],
                        help='Dataset name')
    args = parser.parse_args()
    
    # Combine tree and model name
    model_name = f'{args.tree}_{args.ss_model}'
