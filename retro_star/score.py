"""
RetroStar Synthesis Route Analysis

This script evaluates the feasibility and yield of predicted retrosynthesis routes from RetroStar model.
It processes reaction pathways, calculates success rates, feasibility scores, and yield predictions
for various datasets.

Usage:
    python retro_analysis.py --tree RETRO --ss_model MODEL_NAME --dataset DATASET_NAME
"""
...
def split_in_place_retro(s, delimiter1=".", delimiter2=">>"):
    """
    Split RetroStar reaction SMILES into reactants and product.
    ...
    if delimiter2 in s:
        ...
        if delimiter1 in c:
            reactants = c.split(delimiter1)
        else:
            reactants = [c]
        return (reactants, b)
    return None

def convert_retro_star_route(routes):
    """
    Convert RetroStar routes to reaction list format.
    
    Args:
        routes: Route string with reactions separated by '|'
    ...
    routes = routes.split('|')
    routes = [re.sub(r'>[^>]*>', '>>', route) for route in routes]
    return [split_in_place_retro(route.strip(), '.', '>>') for route in routes]
...
def standardize_pred_keys(pred_file):
    ...
    # Depth (note the different key 'route_len' for RetroStar)
    standardized['depth'] = pred.get('route_lens', pred.get('route_len', []))
    ...
    for pred_file in tqdm(pred_list, desc="Processing prediction files"):
        ...
        for i, route in enumerate(pred['routes']):
            ...
            if success == 1 and time < DEFAULT_TIMEOUT:
                reactions_list = convert_retro_star_route(route)
                if reactions_list:
                    try:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Evaluate RetroStar predictions')
    parser.add_argument('--tree', '-t', default='RETRO',  help='Tree search algorithm')
