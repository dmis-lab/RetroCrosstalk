"""
RetroStar Reaction Pathway Visualization

This script processes RetroStar retrosynthesis route predictions, converts them into tree structures,
...
def convert_retro_star_route(routes):
    """
    Convert RetroStar routes string to list of reaction SMILES.
    
    Args:
        routes: String containing routes separated by '|'
        
    Returns:
        List of reaction SMILES strings or None if input is None
    """
    reactions_list = None
    if routes is not None: 
        routes = routes.split('|')
        reactions_list = [re.sub(r'>[^>]*>', '>>', route) for route in routes]
...
def main():
    """
    Main function to process and visualize RetroStar reaction pathways
    """
    # Configuration
    folder = 'retro_star'
    algorithm = 'RETRO'
    mode = 'pr'  # product>>reactants mode for RetroStar
    ss_model = 'default'
    ...
    for pred in tqdm(pred_list, desc=f"Processing {ss_model} files"):
        pred = pickle.load(open(pred, 'rb'))
        ...
        for i, route in enumerate(routes):
            ...
            route = convert_retro_star_route(route)
            ...
            try:
                ...
                draw_tree_from_path_string(
                    ...
                    save_path=Path(f"{fig_path}/{dataset}/{dataset}_{mol_num-1}_tree"),
                    y_margin=150
                )
